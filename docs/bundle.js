...
async function getKline(code, period = 'day', count = 260) {
  ...
  const json = await cached(`k-${tx}-${p}-${count}`, 30000, () => txJson(url, `k_${tx}_${p}`));
  const node = json?.data?.[tx];
  /* 注意：这里是**返回空数组**而不是抛错。抛错会让整次分析失败、把用户挡在诊断面板前，
     而空 K 线更常见的成因是限流/停牌/次新不足窗口 —— 那些情况下行情、指标、画像
     其实都还能用，只是「K 线与布林/导轨」这一层没数据。
     所以下游拿到空数组时必须自己说清楚原因（见 engine.buildChartPayload 的 degraded），
     不允许静默画一块白 —— 那块白就是用户报的「图像为空」。 */
  if (!node) return [];
  /* 字段名不统一：前复权是 qfqday，不复权是 day；不同周期同理。
     逐个候选键都试一遍，避免接口换个键名就整块图表消失。 */
  const candidates = [
    `qfq${p}`, p,
    ...(p === 'day' ? ['qfqday', 'day', 'qfq', 'kline'] : [])
  ];
  let rows = [];
